...
Line 4: A set of 4 space-delimited gap penalties for introducing gaps in either 
        X or Y, the negative of which will be used in calcs: open_gap_x 
        extend_gap_x open_gap_y extend_gap_y e.g., 0 0.7 0 0.4 
...
Line 9-?: Lines showing the score between an element in X and one
          in Y. 

There are Nx*Ny lines (one for each possible x/y combination) Each line
contains a The output file lists the score of the optimal alignment(s) that
follow Each alignment has one line for X (with necessary gaps) followed by a
line for Y (with necessary gaps). Gaps are represented by "_". If there was
